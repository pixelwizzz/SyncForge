...
function Inbox() {
  return (
    <AppShell crumbs={["SyncForge", "Inbox"]}>
      ...
      <div className="rounded-lg border divide-y" style={{ background: "var(--surface-raised)", borderColor: "var(--border)" }}>
        {ACTIVITY.concat(ACTIVITY).map((a, i) => (
          <div key={i} className="flex items-start gap-3 p-4">
            ...
            <div className="flex-1 text-sm" style={{ color: "var(--ink-secondary)" }}>
              ...
              <div className="font-mono text-[10px] tabular mt-1" style={{ color: "var(--ink-muted)" }}>{a.when}</div>
            </div>
            <span className="w-1.5 h-1.5 rounded-full mt-2" style={{ background: i < 3 ? "var(--accent)" : "transparent" }} />
          </div>
        ))}
      </div>
    </AppShell>
